 ...
 * The constructor takes time proportional to <em>E V</em> (<em>E</em> +
 * <em>V</em>)
...
public class FordFulkerson {
    ...
    private boolean isFeasible(FlowNetwork G, int s, int t) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (FlowEdge e : G.adj(v)) {
                if (e.flow() < -FLOATING_POINT_EPSILON || e.flow() > e
                        .capacity() + FLOATING_POINT_EPSILON) {
                    System.err.println(
                            "Edge does not satisfy capacity constraints: " + e);
                    return false;
                }
            }
        }
