...
public final class StdDraw implements ActionListener, MouseListener,
     ...
     * The pen is circular, so that lines have rounded ends, and when you set
     * the
    ...
    private static void pixel(double x, double y) {
        offscreen.fillRect((int) Math.round(scaleX(x)), (int) Math.round(scaleY(
                y)), 1, 1);
    }
    ...
    public static void picture(double x, double y, String filename,
        ...
        offscreen.rotate(Math.toRadians(-degrees), xs, ys);
        offscreen.drawImage(image, (int) Math.round(xs - ws / 2.0), (int) Math
                .round(ys - hs / 2.0), (int) Math.round(ws), (int) Math.round(
                        hs), null);
        offscreen.rotate(Math.toRadians(+degrees), xs, ys);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (filename != null) {
            StdDraw.save(chooser.getDirectory() + File.separator + chooser
                    .getFile());
        }
    }
