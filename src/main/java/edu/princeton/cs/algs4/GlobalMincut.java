...
public class GlobalMincut {
     ...
     *                                  is less than {@code 2} or if anny edge
     *                                  weight is negative
     */
     ...
     * @param G the edge-weighted graph
     * @throws IllegalArgumentException if the number of vertices of {@code G}
     *                                  is less than {@code 2} or if any edge
     *                                  weight is negative
     */
     ...
     */
    private CutPhase minCutPhase(EdgeWeightedGraph G, boolean[] marked,
            CutPhase cp) {
    ...
    private boolean check(EdgeWeightedGraph G) {
        ...
        if (Math.abs(weight - value) > FLOATING_POINT_EPSILON) {
            System.err.println("Min cut weight = " + weight
                    + " , max flow value = " + value);
            return false;
        }
        return true;
    }
