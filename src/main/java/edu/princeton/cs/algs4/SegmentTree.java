 ...
 * It can be easily customizable to support Range Max Query, Range
 * Multiplication Query etc.
 * <p>
 * Also it has been develop with {@code LazyPropagation} for range updates,
 * which means
 * when you perform update operations over a range, the update process affects
 * the least nodes as possible
 * so that the bigger the range you want to update the less time it consumes to
 * update it. Eventually those changes will be propagated
 ...
 * updated (and not their children) so in this case
...
public class SegmentTree {
    ...
    public SegmentTree(int[] array) {
        ...
        size = (int) (2 * Math.pow(2.0, Math.floor((Math.log(
                (double) array.length) / Math.log(2.0)) + 1)));
        heap = new Node[size];
     ...
     * The values will be propagated lazily from top to bottom of the segment
     * tree.
    ...
    private void update(int v, int from, int to, int value) {
         ...
         * lazily update it.
         * This means We do NOT update each position of the vector, but update
         * only some temporal
         * values into the Node; such values into the Node will be propagated
         * down to its children only when they need to.
         */
        if (contains(from, to, n.from, n.to)) {
            change(n, value);
        }
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if ((!line[0].equals("set") && !line[0].equals("init"))
                    && st == null) {
            ...
            if (line[0].equals("set")) {
                ...
                StdOut.printf("Sum from %d to %d = %d%n", arg1, arg2, st.rsq(
                        arg1, arg2));
            } else if (line[0].equals("rmq")) {
                StdOut.printf("Min from %d to %d = %d%n", arg1, arg2, st.rMinQ(
                        arg1, arg2));
            } else {
