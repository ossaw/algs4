 ...
 * Afterwards, the {@code dist()}, {@code hasPath()}, and
 * {@code hasNegativeCycle()}
...
public class FloydWarshall {
    ...
    public Iterable<DirectedEdge> path(int s, int t) {
        ...
        for (DirectedEdge e = edgeTo[s][t]; e != null; e = edgeTo[s][e
                .from()]) {
            path.push(e);
        }
        return path;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < E; i++) {
            ...
            double weight = Math.round(100 * (StdRandom.uniform() - 0.15))
                    / 100.0;
            if (v == w)
        ...
        // print all-pairs shortest paths
            ...
            for (int v = 0; v < G.V(); v++) {
                for (int w = 0; w < G.V(); w++) {
                    if (spt.hasPath(v, w)) {
                        StdOut.printf("%d to %d (%5.2f)  ", v, w, spt.dist(v,
                                w));
