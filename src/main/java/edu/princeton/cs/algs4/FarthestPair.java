...
public class FarthestPair {
     ...
     * @throws IllegalArgumentException if {@code points} is {@code null} or if
     *                                  any
     *                                  entry in {@code points[]} is
     *                                  {@code null}
     */
    public FarthestPair(Point2D[] points) {
        ...
        int k = 2;
        while (Point2D.area2(hull[m], hull[1], hull[k + 1]) > Point2D.area2(
                hull[m], hull[1], hull[k])) {
            k++;
        }
        ...
        for (int i = 1; i <= k && j <= m; i++) {
            ...
            while ((j < m) && Point2D.area2(hull[i], hull[i + 1], hull[j
                    + 1]) > Point2D.area2(hull[i], hull[i + 1], hull[j])) {
                j++;
                // StdOut.println(hull[i] + " and " + hull[j] + " are antipodal");
     ...
     * This quantity is also known as the <em>diameter</em> of the set of
     * points.
     *
